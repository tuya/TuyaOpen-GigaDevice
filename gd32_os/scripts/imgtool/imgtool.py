...
def do_getpub(args):
    ...
    if args.sha256 == 1:
        sha = hashlib.sha256()
        sha.update(key.get_public_bytes())
        pkhash = sha.digest()
        header="static const uint8_t test_rotpk_hash[] = {"
        trailer="};"
        print(header, end='')
        for count, b in enumerate(pkhash):
            if count % 8 == 0:
                print("\n" + "    ", end='')
            else:
                print(" ", end='')
...
def do_sign(args):
    ...
    img = image.Image.load(args.infile,
                           ...
                           algo_hash=args.algo_hash,
                           algo_sig=args.algo_sig)
#                           pad=args.padto or 0)
    if args.password:
    ...
    cert_key = keys.load(args.cert_key, pwd) if args.cert_key else None
    img.sign(key, args.cert, cert_key)
    #if args.pad:
    #    img.pad_to(padto, 1)
...
def do_pad(args):
    ...
    if args.align is not None:  
        align_sz = int(args.align, 16)
        offset = 0
        if args.offset is not None:
            offset = int(args.offset, 16)
        remain = (len(payload) + offset) % align_sz
        if remain > 0:
            padding = align_sz - remain
...
def args():
    ...
    getpub.add_argument('--sha256', type=int, metavar='hash')
    ...
    sign.add_argument('--algo_hash', metavar='hash')
    sign.add_argument('--algo_sig', metavar='sign')
    ...
    sign.add_argument('-e', '--cert_key', metavar='certkey')
    sign.add_argument("infile")
    sign.add_argument("outfile")
    
    pad = subs.add_parser('pad', help='padding with 0xFF')
    pad.add_argument('-s', '--size', metavar='pad')
    pad.add_argument('-a', '--align', metavar='align')
    pad.add_argument('-o', '--offset', metavar='offset',
                          help='pad after offset + binary')
