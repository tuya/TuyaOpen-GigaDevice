...
/* GD modify */
#ifdef CONFIG_GDWIFI
	enum pbkdf2_digest_alg dig_alg;
#endif /* CONFIG_GDWIFI */
/* GD modify end */
	u8 iv[8];
	size_t iv_len;
...
/* GD modify */
#ifdef CONFIG_GDWIFI
static int digest_alg_is_oid(struct asn1_oid *oid, unsigned long alg)
{
	return oid->len == 6 &&
		oid_is_rsadsi(oid) &&
		oid->oid[4] == 2 /* PKCS#2 */ &&
...
/* GD modify */
	...
	if (pos != kdf_end) {
		if (asn1_get_next(pos, kdf_end - pos, &hdr) < 0 ||
			!asn1_is_sequence(&hdr)) {
			asn1_unexpected(&hdr,
				"PKCS #5: Expected SEQUENCE (digestAlgorithm)");
			return -1;
		}
		...
		if (asn1_get_oid(pos, end - pos, &oid, &pos)) {
			wpa_printf(MSG_DEBUG,
				"PKCS #5: Failed to parse OID (digest algorithm)");
			return -1;
		}

		asn1_oid_to_str(&oid, obuf, sizeof(obuf));
		wpa_printf(MSG_DEBUG, "PKCS #5: PBES2 digest algorithm %s",
			obuf);

		if (digest_alg_is_oid(&oid, 5)) {
			params->dig_alg = PBKDF2_DIG_ALG_MD5;
		} else if (digest_alg_is_oid(&oid, 7)) {
			params->dig_alg = PBKDF2_DIG_ALG_SHA1;
		} else if (digest_alg_is_oid(&oid, 8)) {
			params->dig_alg = PBKDF2_DIG_ALG_SHA224;
		} else if (digest_alg_is_oid(&oid, 9)) {
			params->dig_alg = PBKDF2_DIG_ALG_SHA256;
		} else if (digest_alg_is_oid(&oid, 10)) {
			params->dig_alg = PBKDF2_DIG_ALG_SHA384;
		} else if (digest_alg_is_oid(&oid, 11)) {
			params->dig_alg = PBKDF2_DIG_ALG_SHA512;
		} else {
			wpa_printf(MSG_DEBUG,
				"PKCS #5: Unsupported PBES2 digest algorithm %s",
				obuf);
			return -1;
		}
	}
...
/* GD modify */
#else
    if (crypto_pkcs5_pbkdf2_hmac(params->dig_alg, (const unsigned char *)passwd,
                            strlen(passwd), params->salt, params->salt_len,
