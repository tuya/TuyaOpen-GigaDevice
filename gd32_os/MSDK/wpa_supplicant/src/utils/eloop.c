...
#define assert(exp) do { if (!(exp)) { \
  wpa_printf(MSG_ERROR, "wpa assert"); while (1); }} while(0)
#endif
/* GD modify end */
#include "common.h"
#include "trace.h"
...
#if !defined CONFIG_NATIVE_WINDOWS && !defined CONFIG_GDWIFI /* GD modify */
			  ...
			  void *user_data)
{
	...
	eloop.signal_count++;
	eloop.signals = tmp;
#if !defined CONFIG_GDWIFI /* GD modify */
...
{
/* GD modify */
#if defined CONFIG_NATIVE_WINDOWS || defined CONFIG_GDWIFI
	return 0;
#else
/* GD modify end */
