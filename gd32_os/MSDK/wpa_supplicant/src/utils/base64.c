...
/* GD modify */
	...
	dest = (char *)os_zalloc(len);
	if (crypto_base64_encode((unsigned char *)dest, len, out_len, src, len)) {
		os_free(dest);
		return NULL;
	}
	return dest;
...
/* GD modify */
	...
	dest = (unsigned char *)os_zalloc(len);
	if (crypto_base64_decode(dest, len, out_len, (const unsigned char *)src, len)) {
		os_free(dest);
		return NULL;
	}
	return dest;
