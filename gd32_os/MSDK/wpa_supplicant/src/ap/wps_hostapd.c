...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	fclose(oconf);
 /* GD modify */
	// Fix compile warning: linkr.c:(.text._link_r+0x14): warning: _link is not implemented and will always fail
	// Function rename will call the _link.
	#if 0
  /* GD modify end*/
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (is_nil_uuid(hapd->conf->uuid)) {
		...
		if (uuid && !conf->wps_independent) {
			...
			uuid_gen_by_mac_addr(hapd->own_addr, wps->uuid); /* GD modify */
