...
skip_poll:
	if (next_time) {
		sta->inactive_check_cnt = 0; /* GD modify */
	...
	if (sta->timeout_next == STA_NULLFUNC &&
		...
		sta->inactive_check_cnt++; /* GD modify */
...
/* GD modify */
		if (sta->inactive_check_cnt > 2) {
			...
			sta->timeout_next = STA_NULLFUNC;
			eloop_register_timeout(AP_INACTIVE_CHECK_DELAY, 0, ap_handle_timer, hapd, sta);
		}
/* GD modify end */
		break;
	case STA_DISASSOC:
...
/* GD modify */
		wifi_wpa_send_client_add_event(wifi_wpa_get_vif_idx(hapd->conf->iface),
			sta->addr, WIFI_ALEN);
/* GD modify end*/
...
/* GD modify */
		wifi_wpa_send_client_remove_event(wifi_wpa_get_vif_idx(hapd->conf->iface),
			sta->addr, WIFI_ALEN);
/* GD modify end*/
