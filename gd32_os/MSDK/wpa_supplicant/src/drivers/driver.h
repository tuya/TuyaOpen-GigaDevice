...
struct wpa_driver_ops {
	 ...
	 */
/* GD modify */
	/*	lwip 2.1.2 have a macro named poll in sockets.h, undefine it to avoid compile issue. */
#ifdef poll
	#undef poll
#endif
/* GD modify end */
...
/* GD modify */
	/*
	* suspend_ap - softap stop because another vif in sta mode will run in the different channel. softap need switch it's channel.
	* @vif_idx: softap wifi vif index
	* @freq: channel freq that vif in sta mode will run
	* Returns: 0 on success, -1 on failure
	*/
#ifdef CFG_WIFI_CONCURRENT
	int (*suspend_ap)(void *priv, u8 vif_idx, int freq);
#endif
/* GD modify end */
...
/* GD modify */
#ifdef CONFIG_DRIVER_GDWIFI
extern const struct wpa_driver_ops wpa_driver_gdwifi_ops; /* driver_gdwifi.c */
#endif /* CONFIG_DRIVER_GDWIFI */
/* GD modify end */
#endif /* DRIVER_H */
