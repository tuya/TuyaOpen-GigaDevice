...
static inline int wpa_drv_suspend_ap(struct wpa_supplicant *wpa_s, u8 vif_idx, int freq)
{
	if (!wpa_s->driver->suspend_ap)
		return 0;
	return wpa_s->driver->suspend_ap(wpa_s->drv_priv, vif_idx, freq);
}
#endif
/* GD modify end */
#endif /* DRIVER_I_H */
