...
#include "common/eapol_common.h" /* GD modify */
#include "eapol_supp/eapol_supp_sm.h"
#include "crypto/dh_group5.h"
#include "ap/hostapd.h"
#include "ap/ap_config.h"
#include "ap/wpa_auth.h" /* GD modify */
...
/* GD modify */
	...
	sta = ap_get_sta(wpa_s->ap_iface->bss[0], src_addr);
	if (!sta || (!(sta->flags & (WLAN_STA_ASSOC | WLAN_STA_PREAUTH)) &&
		     !(wpa_s->ap_iface->bss[0]->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED))) {
		wpa_printf(MSG_DEBUG,
	...
	if (datalen >= sizeof(struct ieee802_1x_eapol_key) &&
		...
		wpa_receive(wpa_s->ap_iface->bss[0]->wpa_auth, sta->wpa_sm, (u8 *) hdr,
			    sizeof(*hdr) + datalen);
		return;
	}
