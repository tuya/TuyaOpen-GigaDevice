...
static void wpas_dpp_auth_resp_retry(struct wpa_supplicant *wpa_s)
{
	...
	if (auth->auth_resp_tries >= max_tries) {
		...
		wifi_wpa_send_event_with_name(WIFI_WPA_PROCESS_ERROR, /* GD modify */
					       (void *)WIFI_WPA_ERROR_DPP, 0 , wpa_s->ifname);
				   ...
				   struct dpp_config_obj *conf)
{
	...
	wifi_wpa_send_event_with_name(WIFI_WPA_DPP_NETWORK_ADDED, /* GD modify */
				       (void *)ssid->id, 0 , wpa_s->ifname);
