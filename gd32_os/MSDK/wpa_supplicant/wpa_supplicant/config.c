...
#ifdef CONFIG_USE_FULL_SSID_FIELDS
	{ FUNC(bssid_hint) },
	{ FUNC(bssid_ignore) },
	{ FUNC(bssid_accept) },
...
#ifndef CONFIG_NO_EDMG_SUPPORT
	{ INT_RANGE(enable_edmg, 0, 1) },
	{ INT_RANGE(edmg_channel, 9, 13) },
#endif /* CONFIG_NO_EDMG_SUPPORT */
#ifdef CONFIG_ACS
	{ INT_RANGE(acs, 0, 1) },
#endif /* CONFIG_ACS */
#ifdef CONFIG_MESH
...
#endif /* CONFIG_USE_FULL_SSID_FIELDS */
};
...
void wpa_config_free(struct wpa_config *config)
{
	...
	os_free(config->p2p_ssid_postfix); /* GD modify */
 ...
 */
/* GD modify */
int config_sae_groups[4] = {0};
char *config_non_pref_chan = NULL;
/* GD modify end */
...
#undef ecw2cw
	...
	config->p2p_search_delay = DEFAULT_P2P_SEARCH_DELAY; /* GD modify */
#endif /* CONFIG_P2P */ /* GD modify */
...
/* GD modify */
	config->sae_pwe = 2;
	if(config_sae_groups[0] != 0) {
		config->sae_groups = os_zalloc(4 * sizeof(int));
		if (config->sae_groups != NULL) {
			os_memcpy((uint8_t *)config->sae_groups, (uint8_t *)config_sae_groups, 4 * sizeof(int));
		} else {
			wpa_printf(MSG_WARNING, "Malloc sae groups failed");
		}
	}
#ifdef CONFIG_MBO
	if (config_non_pref_chan != NULL) {
		config->non_pref_chan = config_non_pref_chan;
	}
#endif /* CONFIG_MBO */
/* GD modify end */
	if (ctrl_interface)
...
/* GD modify */
/**
 * wpa_config_set_sae_pwe - Set SAE mechanism for PWE derivation
 * @config: Configuration data.
 * @sae_pwe: SAE mechanism.
 * 0 = hunting-and-pecking loop only
 * 1 = hash-to-element only
 * 2 = both hunting-and-pecking loop and hash-to-element enabled
 */
void wpa_config_set_sae_pwe(struct wpa_config *config, int sae_pwe)
{
    config->sae_pwe = sae_pwe;
}
/* GD modify end */
