...
#elif !defined(__CYGWIN__) && !defined(CONFIG_NATIVE_WINDOWS) && !defined(CONFIG_GDWIFI) /* GD modify */
...
#ifndef CONFIG_WPA_CLI_DISABLE /* GD modify */
...
#endif /* CONFIG_WPA_CLI_DISABLE */ /* GD modify */
...
/* GD modify */
	if (ret == 0 && ssid->mode == WPAS_MODE_AP)
		wpa_config_set_sae_pwe(wpa_s->conf, 0);
/* GD modify end */
	return ret;
}
...
#ifndef CONFIG_GDWIFI /* GD modify */
...
char * wpa_supplicant_ctrl_iface_process(struct wpa_supplicant *wpa_s,
					 ...
					 char *reply)
{
	const int reply_size = *resp_len;
	int reply_len;

	reply_len = os_snprintf(reply, reply_size, "OK");

	if (os_strncmp(buf, "ENABLE_NETWORK ", 15) == 0) {
		if (wpa_supplicant_ctrl_iface_enable_network(wpa_s, buf + 15))
			reply_len = -1;
	} else if (os_strncmp(buf, "DISABLE_NETWORK ", 16) == 0) {
		if (wpa_supplicant_ctrl_iface_disable_network(wpa_s, buf + 16))
...
int wpa_supplicant_global_iface_remove(struct wpa_global *global, /* GD modify */
					      char *cmd)
{
	struct wpa_supplicant *wpa_s;
	int ret;
...
char * wpa_supplicant_global_ctrl_iface_process(struct wpa_global *global,
						char *buf, size_t *resp_len, char *reply)
{
	const int reply_size = *resp_len;
	int reply_len = 0;
	int level = MSG_DEBUG;
