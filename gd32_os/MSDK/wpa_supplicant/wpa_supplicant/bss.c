...
{
	const u8 * dsps = NULL; /* GD modify */

	dst->flags = src->flags;
	os_memcpy(dst->bssid, src->bssid, ETH_ALEN);
/* GD modify */
	dsps = wpa_scan_get_ie(src, WLAN_EID_DS_PARAMS);
	if (dsps == NULL) {
		dst->freq = src->freq; //for 5G band
	} else {
		// for 2.4G band
		if (14 == *(dsps + MAC_IE_ELEMENT_OFFSET))
			dst->freq = 2484;
		else
			dst->freq = (*(dsps + MAC_IE_ELEMENT_OFFSET) * 5) + 2407;
	}
/* GD modify end */
