...
#ifdef CONFIG_HS20 /* GD modify */
	if (ssid->key_mgmt & WPA_KEY_MGMT_OSEN)
		privacy = 1;
#endif /*CONFIG_HS20*/ /* GD modify */
					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	while ((ssid->proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN)) && rsn_ie) {
		...
		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip RSN IE - key mgmt mismatch (%d - %d)", ie.key_mgmt, ssid->key_mgmt); /* GD modify */
			break;
		}
					      ...
					      union wpa_event_data *data)
{
	...
	if (selected) {
		...
		wpa_drv_suspend_ap(wpa_s, WIFI_VIF_INDEX_SOFTAP_MODE, selected->freq); /* GD modify */
		#endif
...
/* GD modify */
static void wpas_event_rssi_status(struct wpa_supplicant * wpa_s,
                                    void *temp)
{
    if (wpa_s->wpa_state == WPA_COMPLETED) {
        wpa_supplicant_req_scan(wpa_s, 0, 100);
    }
}
/* GD modify end */
...
/* GD modify */
	if (wpa_s->wpa_state <= WPA_INTERFACE_DISABLED) {
		wpa_dbg(wpa_s, MSG_INFO, "Ignore disassoc event when interface disabled or disconnected");
		return;
	}
#ifdef CONFIG_MBO
	if (wpa_s->reassoc_same_ess == 1 && wpa_s->wpa_state == WPA_ASSOCIATING) {
		wpa_dbg(wpa_s, MSG_INFO, "Ignore disassoc event when interface reassociating and in WPA_ASSOCIATING state");
		return;
	}
#endif /* CONFIG_MBO */
/* GD modify end */
...
/* GD modify */
	if (wpa_s->wpa_state <= WPA_INTERFACE_DISABLED) {
		wpa_dbg(wpa_s, MSG_INFO, "Ignore deauth event when interface disabled or disconnected");
		return;
	}
/* GD modify end */
...
#ifndef CONFIG_AP_NO_DFS /* GD modify */
	if (wpa_s->ap_iface || wpa_s->ifmsh) {
		wpas_ap_event_dfs_cac_started(wpa_s, radar);
	} else
#endif /* CONFIG_AP_NO_DFS */ /* GD modify */
 ...
 /* GD modify */
	...
	case EVENT_SIGNAL_CHANGE:
		...
		wpa_bss_update_level(wpa_s->current_bss,
				     data->signal_change.current_signal);
		wpas_event_rssi_status(wpa_s, data);
	...
	case EVENT_FT_RESPONSE:
		...
		break;
#endif /* CONFIG_IEEE80211R */
#ifndef CONFIG_REMOVE_UNUSED_EVENTS
	case EVENT_AUTH:
#ifdef CONFIG_FST
		if (!wpas_fst_update_mbie(wpa_s, data->auth.ies,
					  data->auth.ies_len))
...
case EVENT_MICHAEL_MIC_FAILURE:
	...
	case EVENT_UNPROT_BEACON:
		...
		break;
#endif /* CONFIG_REMOVE_UNUSED_EVENTS */ /* GD modify */
	default:
