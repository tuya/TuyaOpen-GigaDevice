...
int rt_memheap_dump(void)
{
    ...
    for (node = list->next; node != list; node = node->next)
    {
        ...
        /* get the first free memory block */
        header_ptr = mh->free_list->next_free;
        for (header_ptr = mh->free_list->next_free;header_ptr != mh->free_list; header_ptr = header_ptr->next_free, index++)
        {
            ...
            rt_kprintf("[%d]=0x%08x, %d\r\n", index, header_ptr, block_size);
#if 0
