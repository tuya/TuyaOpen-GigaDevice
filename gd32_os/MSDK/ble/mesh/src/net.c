...
static struct k_mem_slab loopback_buf_pool = {
	.sema_count = NULL,
	.num_blocks = CONFIG_BT_MESH_LOOPBACK_BUFS,
	.block_size = sizeof(struct loopback_buf),
};
...
static int pre_net_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return net_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_net = {
	.name = "Net",
	.h_get = NULL,
	.h_set = pre_net_set,
	.h_commit = NULL,
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_iv_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return iv_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_iv = {
	.name = "IV",
	.h_get = NULL,
	.h_set = pre_iv_set,
	.h_commit = NULL,
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_seq_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return seq_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_seq = {
	.name = "Seq",
	.h_get = NULL,
	.h_set = pre_seq_set,
	.h_commit = NULL,
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_dev_key_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return dev_key_cand_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_dev_key = {
	.name = "DevKeyC",
	.h_get = NULL,
	.h_set = pre_dev_key_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
void bt_mesh_net_pending_dev_key_cand_store(void)
{
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_DEVKEY_CAND)) {
		err = settings_save_one("DevKeyC", &bt_mesh.dev_key_cand,
					sizeof(struct bt_mesh_key));
	} else {
		err = settings_delete("DevKeyC");
	}
...
static void store_pending_iv(void)
{
	...
	err = settings_save_one("IV", &iv, sizeof(iv));
	if (err) {
...
static void store_pending_net(void)
{
	...
	err = settings_save_one("Net", &net, sizeof(net));
	if (err) {
...
void bt_mesh_net_pending_seq_store(void)
{
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
		...
		err = settings_save_one("Seq", &seq, sizeof(seq));
		if (err) {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_net_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_net);
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_iv);
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_seq);
#if defined(CONFIG_BT_MESH_RPR_SRV)
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_dev_key);
#endif
}
#endif
