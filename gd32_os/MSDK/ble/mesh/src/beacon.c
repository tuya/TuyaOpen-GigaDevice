...
// Work around for private beacon test
#define PROV_XMIT                  BT_MESH_TRANSMIT(10, 20)
#else
...
static struct bt_mesh_beacon_cb *p_beacon_list = NULL;
void bt_mesh_beacon_cb_register(struct bt_mesh_beacon_cb *p_beacon_cb)
{
	struct bt_mesh_beacon_cb  *p_cur_beacon_cb = NULL;
	if (p_beacon_cb) {
		if (p_beacon_list == NULL) {
			p_beacon_list = p_beacon_cb;
			p_beacon_cb->next = NULL;
		}
		else {
			p_cur_beacon_cb = p_beacon_list;
			while(p_cur_beacon_cb->next != NULL) {
				p_cur_beacon_cb = p_cur_beacon_cb->next;
			}
			p_cur_beacon_cb->next = p_beacon_cb;
			p_beacon_cb->next = NULL;
		}
	}
}
#else
extern uint32_t _beacon_cbs[];
extern uint32_t _ebeacon_cbs[];
#endif
#endif
				 ...
				 struct net_buf_simple *buf)
{
	...
	adv = CONTAINER_OF(buf, struct bt_mesh_adv, b);
	adv->ctx.priv = 1;
...
static bool secure_beacon_authenticate(struct bt_mesh_subnet *sub, void *cb_data)
{
	...
	for (int i = 0; i < ARRAY_SIZE(sub->keys); i++) {
		if (sub->keys[i].valid && auth_match(&sub->keys[i], params)) {
			...
			beacon_info.net_idx = sub->net_idx;
...
#if (CONFIG_MESH_CB_REGISTERED)
			struct bt_mesh_beacon_cb *p_cur_beacon_cb = p_beacon_list;

			while(p_cur_beacon_cb) {
				if (p_cur_beacon_cb->snb_received) {
					p_cur_beacon_cb->snb_received(&beacon_info);
				}
				p_cur_beacon_cb = p_cur_beacon_cb->next;
			}
#else
			struct bt_mesh_beacon_cb *p_cur_beacon_cb = (struct bt_mesh_beacon_cb *)_beacon_cbs;

			while(p_cur_beacon_cb != (struct bt_mesh_beacon_cb *)_ebeacon_cbs) {
				if (p_cur_beacon_cb->snb_received) {
					p_cur_beacon_cb->snb_received(&beacon_info);
				}
				p_cur_beacon_cb++;
...
static bool priv_beacon_decrypt(struct bt_mesh_subnet *sub, void *cb_data)
{
	...
	for (int i = 0; i < ARRAY_SIZE(sub->keys); i++) {
		...
		if (!err) {
			...
			while(p_cur_beacon_cb) {
				if (p_cur_beacon_cb->priv_received) {
					p_cur_beacon_cb->priv_received(&beacon_info);
				}
				p_cur_beacon_cb = p_cur_beacon_cb->next;
			}
#else
...
#if (CONFIG_MESH_CB_REGISTERED)
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_beacon = {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_beacon_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_beacon);
}
#endif
