...
static struct bt_mesh_app_key_cb *p_key_list = NULL;
#else
extern uint32_t _appkeys_cbs[];
extern uint32_t _eappkeys_cbs[];
#endif
...
static void clear_app_key(uint16_t app_idx)
{
	...
	snprintf(path, sizeof(path), "AppKey/%x", app_idx);
...
#if (CONFIG_MESH_CB_REGISTERED)
	struct bt_mesh_app_key_cb *p_cur_key_cb = NULL;
	p_cur_key_cb = p_key_list;
	while(p_cur_key_cb) {
		p_cur_key_cb->evt_handler(app->app_idx, app->net_idx, evt);
		p_cur_key_cb = p_cur_key_cb->next;
	}
#else
	struct bt_mesh_app_key_cb *p_cur_key_cb = (struct bt_mesh_app_key_cb *)_appkeys_cbs;

	while(p_cur_key_cb != ((struct bt_mesh_app_key_cb *)_eappkeys_cbs)) {
		p_cur_key_cb->evt_handler(app->app_idx, app->net_idx, evt);
		p_cur_key_cb++;
	}
#endif
...
#if (CONFIG_MESH_CB_REGISTERED)
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_app_keys = {
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_app_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return app_key_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_app = {
	.name = "AppKey",
	.h_get = NULL,
	.h_set = pre_app_set,
	.h_commit = NULL,
...
void bt_mesh_app_key_cb_register(struct bt_mesh_app_key_cb *p_key_cb)
{
	...
	if (p_key_cb) {
		if (p_key_list == NULL) {
			...
			p_cur_key_cb = p_key_list;
			while(p_cur_key_cb->next != NULL) {
				p_cur_key_cb = p_cur_key_cb->next;
			}
			p_cur_key_cb->next = p_key_cb;
			p_key_cb->next = NULL;
		}
	}
}
...
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_app);
}

extern void bt_mesh_cfg_srv_app_key_reg_init(void);
void bt_mesh_app_keys_init(void)
{
	bt_mesh_cfg_srv_app_key_reg_init();
}

void bt_mesh_app_keys_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_app_keys);
}
#endif
