 ...
 */
//#define CONFIG_BT_MESH_LOW_POWER_LOG_LEVEL_DBG
...
static struct bt_mesh_lpn_cb *p_lpn_cb_list = NULL;
#else
extern uint32_t _lpn_cbs[];
extern uint32_t _elpn_cbs[];
#endif
...
static void clear_friendship(bool force, bool disable)
{
	...
	if (was_established) {
#if (CONFIG_MESH_CB_REGISTERED)
		struct bt_mesh_lpn_cb *p_cur_lpn_cb = p_lpn_cb_list;
		while(p_cur_lpn_cb) {
			if (p_cur_lpn_cb->terminated) {
				p_cur_lpn_cb->terminated(net_idx, frnd);
			}

			p_cur_lpn_cb = p_cur_lpn_cb->next;
		}
#else
		struct bt_mesh_lpn_cb *p_cur_lpn_cb = (struct bt_mesh_lpn_cb *)_lpn_cbs;
		while(p_cur_lpn_cb != (struct bt_mesh_lpn_cb *)_elpn_cbs) {
			if (p_cur_lpn_cb->terminated) {
				p_cur_lpn_cb->terminated(net_idx, frnd);
			}
...
static void req_send_end(int err, void *user_data)
{
	...
	while(p_cur_lpn_cb) {
		if (p_cur_lpn_cb->polled) {
			p_cur_lpn_cb->polled(lpn->sub->net_idx, lpn->frnd, retry);
		}
			      ...
			      struct net_buf_simple *buf)
{
	...
	if (established) {
		...
		struct bt_mesh_lpn_cb *p_cur_lpn_cb = p_lpn_cb_list;
		while(p_cur_lpn_cb) {
			if (p_cur_lpn_cb->established) {
				p_cur_lpn_cb->established(lpn->sub->net_idx, lpn->frnd, lpn->queue_size,
						lpn->recv_win);
			}
...
#if (CONFIG_MESH_CB_REGISTERED)
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_lpn = {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_lpn_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_lpn);
}

void bt_mesh_lpn_cb_register(struct bt_mesh_lpn_cb *p_lpn_cb)
{
	struct bt_mesh_lpn_cb *p_cur_lpn_cb = NULL;
	if (p_lpn_cb) {
		if (p_lpn_cb_list == NULL) {
			p_lpn_cb_list = p_lpn_cb;
			p_lpn_cb->next = NULL;
		}
		else {
			p_cur_lpn_cb = p_lpn_cb_list;
			while(p_cur_lpn_cb->next != NULL) {
				p_cur_lpn_cb = p_cur_lpn_cb->next;
			}
			p_cur_lpn_cb->next = p_lpn_cb;
			p_lpn_cb->next = NULL;
		}
	}
}
