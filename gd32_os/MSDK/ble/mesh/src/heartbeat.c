...
static struct bt_mesh_hb_cb  *p_heartbeat_cb_list = NULL;
#else
extern uint32_t _hb_cbs[];
extern uint32_t _ehb_cbs[];
#endif

static void notify_pub_sent(void)
{
#if (CONFIG_MESH_CB_REGISTERED)
	struct bt_mesh_hb_cb *p_cur_heartbeat_cb = p_heartbeat_cb_list;

	while(p_cur_heartbeat_cb) {
		if (p_cur_heartbeat_cb->pub_sent) {
			p_cur_heartbeat_cb->pub_sent(&pub);
		}
		p_cur_heartbeat_cb = p_cur_heartbeat_cb->next;
	}
#else
	struct bt_mesh_hb_cb *p_cur_heartbeat_cb = (struct bt_mesh_hb_cb *)_hb_cbs;

	while(p_cur_heartbeat_cb != (struct bt_mesh_hb_cb *)_ehb_cbs) {
		if (p_cur_heartbeat_cb->pub_sent) {
			p_cur_heartbeat_cb->pub_sent(&pub);
		}
		p_cur_heartbeat_cb++;
...
static void notify_recv(uint8_t hops, uint16_t feat)
{
	...
	while(p_cur_heartbeat_cb) {
		if (p_cur_heartbeat_cb->recv) {
			p_cur_heartbeat_cb->recv(&sub, hops, feat);
		}
...
static void notify_sub_end(void)
{
	...
	while(p_cur_heartbeat_cb) {
		if (p_cur_heartbeat_cb->sub_end) {
			p_cur_heartbeat_cb->sub_end(&sub);
		}
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_pub_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return hb_pub_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_pub = {
	.name = "HBPub",
	.h_get = NULL,
	.h_set = pre_pub_set,
	.h_commit = NULL,
...
void bt_mesh_hb_pub_pending_store(void)
{
	...
	if (hb_pub.dst == BT_MESH_ADDR_UNASSIGNED) {
		...
		err = settings_save_one("HBPub", &val, sizeof(val));
	}
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_hb_pub_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_pub);
}

void bt_mesh_hearbeat_cb_register(struct bt_mesh_hb_cb *p_heartbeat_cb)
{
	struct bt_mesh_hb_cb  *p_cur_subnet_cb = NULL;
	if (p_heartbeat_cb) {
		if (p_heartbeat_cb_list == NULL) {
			p_heartbeat_cb_list = p_heartbeat_cb;
			p_heartbeat_cb->next = NULL;
		}
		else {
			p_cur_subnet_cb = p_heartbeat_cb_list;
			while(p_cur_subnet_cb->next != NULL) {
				p_cur_subnet_cb = p_cur_subnet_cb->next;
			}
			p_cur_subnet_cb->next = p_heartbeat_cb;
			p_heartbeat_cb->next = NULL;
		}
	}
}
