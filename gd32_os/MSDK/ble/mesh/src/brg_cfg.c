...
static atomic_t brg_cfg_flags[ATOMIC_BITMAP_SIZE(BRG_CFG_FLAGS_COUNT)];
...
static int pre_brg_en_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return brg_en_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_brg_en = {
	.name = "brg_en",
	.h_get = NULL,
	.h_set = pre_brg_en_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_brg_tbl_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return brg_tbl_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_brg_tbl = {
	.name = "brg_tbl",
	.h_get = NULL,
	.h_set = pre_brg_tbl_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_sbr = {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_brg_cfg_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_brg_en);
  bt_mesh_settings_cb_register(&settings_handler_bt_mesh_brg_tbl);
}

void bt_mesh_brg_cfg_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_sbr);
}
#endif
#endif
