...
#define BT_MESH_RPR_UNPROV_BEACON       BIT(0)
#define BT_MESH_RPR_UNPROV_GATT         BIT(1)
const struct prov_bearer *p_bt_mesh_pb = &bt_mesh_pb_adv;

#if (CONFIG_BT_TESTING)
static bool force_2_use_pb_gatt = false;
#endif
...
static struct {
	...
	atomic_t flags[ATOMIC_BITMAP_SIZE(RPR_SRV_NUM_FLAGS)];
		       ...
		       enum prov_bearer_link_status reason)
{
	...
	if (atomic_test_and_clear_bit(srv.flags, NODE_REFRESH)) {
		...
		p_bt_mesh_pb->link_close(reason);
	}
}
...
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_rpr_srv = {
			   ...
			   enum prov_bearer_link_status reason)
{
	...
	LOG_DBG("reason %u, state %d", reason, srv.link.state);
	...
	} else if ((reason == PROV_BEARER_LINK_STATUS_FAIL || bearer->type == BT_MESH_PROV_GATT) &&
		  ...
		  struct net_buf_simple *buf)
{
	...
	if (err) {
		...
		link_close(BT_MESH_RPR_ERR_LINK_CLOSED_AS_CANNOT_SEND_PDU,
			   PROV_BEARER_LINK_STATUS_FAIL);
		p_bt_mesh_pb->link_close(PROV_ERR_RESOURCES);
	}
}
				      ...
				      struct net_buf_simple *buf)
{
	...
	if ((mesh_log_mask[LOG_LEVEL >> 1] >> 4) > 3) {
			    ...
			    struct net_buf_simple *buf)
{
	...
	if (!srv.link.dev) {
		...
		srv.link.dev->unprov_type = 0;
	}

#if CONFIG_BT_MESH_PB_GATT_COMMON
	if ((srv.link.dev->unprov_type & BT_MESH_RPR_UNPROV_GATT)
#if CONFIG_BT_TESTING
		  || (srv.link.dev->unprov_type == 0 && force_2_use_pb_gatt)
#endif
		  ) {
		p_bt_mesh_pb = &bt_mesh_pb_gatt;
	} else {
		p_bt_mesh_pb = &bt_mesh_pb_adv;
	}
#endif

	err = p_bt_mesh_pb->link_open(srv.link.dev->uuid, timeout, &prov_bearer_cb, &srv);
	if (err) {
		status = BT_MESH_RPR_ERR_LINK_CANNOT_OPEN;
		goto rsp;
	}
			   ...
			   struct net_buf_simple *buf)
{
	...
	if (atomic_test_bit(srv.flags, NODE_REFRESH)) {
		...
		srv.link.state = BT_MESH_RPR_LINK_SENDING;
		err = p_bt_mesh_pb->send(buf, pdu_send_complete, &srv);
		if (err) {
			link_close(
				BT_MESH_RPR_ERR_LINK_CLOSED_AS_CANNOT_SEND_PDU,
				PROV_BEARER_LINK_STATUS_FAIL);
		  ...
		  struct bt_data *ad)
{
	...
	dev->unprov_type |= BT_MESH_RPR_UNPROV_BEACON;
	memcpy(uuid_repr.val, uuid, 16);
...
static struct bt_mesh_rpr_unprov *
adv_handle_prov_svc(const struct bt_le_scan_recv_info *info,
		  struct bt_data *ad)
{
    struct bt_uuid_128 uuid_repr = { .uuid = { BT_UUID_TYPE_128 } };
    struct bt_mesh_rpr_unprov *dev = NULL;
    ...
    if (*((uint16_t *)ad->data) != BT_UUID_MESH_PROV_VAL || ad->data_len != 20)
    {
        return NULL;
    }
    ...
    dev->oob = sys_get_be16(&ad->data[18]);
    dev->rssi = info->rssi;

    dev->flags |= BT_MESH_RPR_UNPROV_FOUND;
    dev->unprov_type |= BT_MESH_RPR_UNPROV_GATT;
    memcpy(uuid_repr.val, uuid, 16);

    LOG_DBG("Unprov #%u: %s OOB: 0x%04x (no hash)", dev - &srv.scan.devs[0],
            bt_uuid_str(&uuid_repr.uuid), dev->oob);
				...
				struct net_buf_simple *buf)
{
	...
	while (pull_ad_data(buf, &ad)) {
		...
		if (ad.type == BT_DATA_MESH_BEACON && !dev) {
			...
			if (bt_mesh_s1((const char *)ad.data, ad.data_len, hash) ||
			...
			dev = srv.scan.dev;
			srv.scan.dev->flags |= BT_MESH_RPR_UNPROV_EXT_ADV_RXD;
		} else if (ad.type == BT_DATA_SVC_DATA16 && !dev) {
			dev = adv_handle_prov_svc(info, &ad);
			is_beacon = false;
			srv.scan.dev->flags |= BT_MESH_RPR_UNPROV_EXT_ADV_RXD;
		}
	}
			    ...
			    struct net_buf_simple *buf)
{
	...
	if (info->adv_type != BT_HCI_ADV_NONCONN_IND && info->adv_type != BT_HCI_ADV_IND) {
		return;
	}
	...
	while (pull_ad_data(buf, &ad)) {
		...
		else if (ad.type == BT_DATA_SVC_DATA16){
			adv_handle_prov_svc(info, &ad);
		}
	}
}
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_rpr_srv_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_rpr_srv);
}
#endif

#if (CONFIG_BT_TESTING)
void bt_mesh_force_2_use_pb_gatt(bool use)
{
	force_2_use_pb_gatt = use;
}
#endif
