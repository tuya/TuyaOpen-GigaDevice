...
struct bt_mesh_friend {
	...
	struct bt_mesh_friend_seg seg[FRIEND_SEG_RX];
...
struct bt_mesh_lpn {
	...
	atomic_t added[ATOMIC_BITMAP_SIZE(LPN_GROUPS)];
	//ATOMIC_DEFINE(pending, LPN_GROUPS);
	atomic_t pending[ATOMIC_BITMAP_SIZE(LPN_GROUPS)];
	//ATOMIC_DEFINE(to_remove, LPN_GROUPS);
	atomic_t to_remove[ATOMIC_BITMAP_SIZE(LPN_GROUPS)];
};
...
struct bt_mesh_net {
	...
	atomic_t flags[ATOMIC_BITMAP_SIZE(BT_MESH_FLAG_COUNT)];
