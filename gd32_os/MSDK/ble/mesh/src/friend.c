...
static struct bt_mesh_friend_cb *p_friend_cb_list = NULL;
#else
extern uint32_t _friend_cbs[];
extern uint32_t _efriend_cbs[];
#endif
...
static struct net_buf friend_buf_data_buf_pool[FRIEND_BUF_COUNT] = {0};
static struct net_buf_pool friend_buf_pool = {
	.mutex = NULL,
	.buf_count = FRIEND_BUF_COUNT,
	.uninit_count = FRIEND_BUF_COUNT,
	.data_size = BT_MESH_ADV_DATA_SIZE,
	.__bufs = (struct net_buf *)friend_buf_data_buf_pool,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
	struct bt_mesh_friend_cb *p_cur_friend_cb = p_friend_cb_list;

	while(p_cur_friend_cb) {
		if (frnd->established && p_cur_friend_cb->terminated) {
			p_cur_friend_cb->terminated(frnd->subnet->net_idx, frnd->lpn);
		}
		p_cur_friend_cb = p_cur_friend_cb->next;
	}
#else
	struct bt_mesh_friend_cb *p_cur_friend_cb = (struct bt_mesh_friend_cb *)_friend_cbs;

	while(p_cur_friend_cb != (struct bt_mesh_friend_cb *)_efriend_cbs) {
		if (frnd->established && p_cur_friend_cb->terminated) {
			p_cur_friend_cb->terminated(frnd->subnet->net_idx, frnd->lpn);
		}
		p_cur_friend_cb++;
...
int bt_mesh_friend_poll(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
	...
	while(p_cur_friend_cb) {
		if (p_cur_friend_cb->polled) {
			p_cur_friend_cb->polled(frnd->subnet->net_idx, frnd->lpn);
		}
		p_cur_friend_cb = p_cur_friend_cb->next;
	}
#else
...
#if (CONFIG_MESH_CB_REGISTERED)
		p_cur_friend_cb = p_friend_cb_list;
		while(p_cur_friend_cb) {
			if (p_cur_friend_cb->established) {
				p_cur_friend_cb->established(frnd->subnet->net_idx, frnd->lpn, frnd->recv_delay,
					  	frnd->poll_to);
			}
			p_cur_friend_cb = p_cur_friend_cb->next;
		}
#else
		p_cur_friend_cb = (struct bt_mesh_friend_cb *)_friend_cbs;
		while(p_cur_friend_cb != (struct bt_mesh_friend_cb *)_efriend_cbs) {
			if (p_cur_friend_cb->established) {
				p_cur_friend_cb->established(frnd->subnet->net_idx, frnd->lpn, frnd->recv_delay,
					  	frnd->poll_to);
...
#if (CONFIG_MESH_CB_REGISTERED)
static struct bt_mesh_subnet_cb  bt_mesh_subnet_cb_friend = {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_friend_cb_register(struct bt_mesh_friend_cb *p_friend_cb)
{
	struct bt_mesh_friend_cb *p_cur_friend_cb = NULL;
	if (p_friend_cb) {
		if (p_friend_cb_list == NULL) {
			p_friend_cb_list = p_friend_cb;
			p_friend_cb->next = NULL;
		}
		else {
			p_cur_friend_cb = p_friend_cb_list;
			while(p_cur_friend_cb->next != NULL) {
				p_cur_friend_cb = p_cur_friend_cb->next;
			}
			p_cur_friend_cb->next = p_friend_cb;
			p_friend_cb->next = NULL;
		}
	}
}

void bt_mesh_friend_subnet_cb_init(void)
{
	bt_mesh_subnet_cb_register(&bt_mesh_subnet_cb_friend);
}
#endif
