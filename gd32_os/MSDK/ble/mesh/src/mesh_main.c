		 ...
		 const struct bt_mesh_comp *comp)
{
	...
	if (err) {
		LOG_ERR("bt_mesh_init crypto err %d", err);
		return err;
	}

	err = bt_mesh_comp_register(comp);
	if (err) {
		LOG_ERR("bt_mesh_init comp_register err %d", err);
		return err;
	}
	...
	if (IS_ENABLED(CONFIG_BT_MESH_PROV)) {
		...
		if (err) {
			LOG_ERR("bt_mesh_init prov_init err %d", err);
			return err;
		}
	}

#if (CONFIG_MESH_CB_REGISTERED)
	bt_mesh_app_keys_init();
	bt_mesh_subnet_init();
#endif
	bt_mesh_cfg_default_set();
	bt_mesh_net_init();
