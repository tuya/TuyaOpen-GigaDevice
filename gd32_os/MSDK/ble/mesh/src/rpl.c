...
static atomic_t store[ATOMIC_BITMAP_SIZE(CONFIG_BT_MESH_CRPL)];
...
static atomic_t rpl_flags[ATOMIC_BITMAP_SIZE(RPL_FLAGS_COUNT)];
...
static void clear_rpl(struct bt_mesh_rpl *rpl)
{
	...
	snprintf(path, sizeof(path), "RPL/%x", rpl->src);
...
static int pre_rpl_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return rpl_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_rpl = {
	.name = "RPL",
	.h_get = NULL,
	.h_set = pre_rpl_set,
	.h_commit = NULL,
...
static void store_rpl(struct bt_mesh_rpl *entry)
{
	...
	snprintf(path, sizeof(path), "RPL/%x", entry->src);
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_rpl_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_rpl);
}
#endif
