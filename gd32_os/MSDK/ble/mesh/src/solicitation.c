...
#define LOG_LEVEL CONFIG_BT_MESH_SOLICI_MODEL_LOG_LEVEL
...
static atomic_t store[ATOMIC_BITMAP_SIZE(CONFIG_BT_MESH_PROXY_SRPL_SIZE)];
...
static int pre_sseq_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return sseq_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_sseq = {
	.name = "SSeq",
	.h_get = NULL,
	.h_set = pre_sseq_set,
	.h_commit = NULL,
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_srpl_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return srpl_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_srpl = {
	.name = "SRPL",
	.h_get = NULL,
	.h_set = pre_srpl_set,
	.h_commit = NULL,
...
static void srpl_entry_clear(int i)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		snprintf(path, sizeof(path), "SRPL/%x", addr);
...
static void srpl_store(struct srpl_entry *entry)
{
	...
	snprintf(path, sizeof(path), "SRPL/%x", entry->ssrc);
...
int bt_mesh_sol_send(void)
{
	...
	err = bt_mesh_adv_bt_data_send(BT_MESH_TRANSMIT_COUNT(CONFIG_BT_MESH_SOL_ADV_XMIT) + 1,
				       adv_int, ad, 3);
	if (err) {
	...
	return adv_int * (BT_MESH_TRANSMIT_COUNT(CONFIG_BT_MESH_SOL_ADV_XMIT) + 1);
#else
	return -ENOTSUP;
#endif
}
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_sol_settings_init(void)
{
#if (CONFIG_BT_MESH_PROXY_SOLICITATION)
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_sseq);
#endif
#if (CONFIG_BT_MESH_OD_PRIV_PROXY_SRV)
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_srpl);
#endif
}
#endif
