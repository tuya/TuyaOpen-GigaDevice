...
static int pre_va_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return va_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_va = {
	.name = "Va",
	.h_get = NULL,
	.h_set = pre_va_set,
	.h_commit = NULL,
...
void bt_mesh_va_pending_store(void)
{
	...
	for (i = 0; (lab = va_get_by_idx(i)) != NULL; i++) {
		...
		snprintf(path, sizeof(path), "Va/%x", i);
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_va_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_va);
}
#endif
