...
struct pb_adv {
	...
	atomic_t flags[ATOMIC_BITMAP_SIZE(ADV_NUM_FLAGS)];
	...
	struct {
		 ...
		 */
		struct unacked_adv_ctx unacked[2];
...
static struct k_work_delayable tx_work  = Z_WORK_DELAYABLE_INITIALIZER(tx_work_handler);
