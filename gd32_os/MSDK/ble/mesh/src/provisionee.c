...
static void prov_start(const uint8_t *data)
{
	...
	if (IS_ENABLED(CONFIG_BT_MESH_OOB_AUTH_REQUIRED) &&
		...
		LOG_ERR("Authentication Not required, oob_method: 0x%02x, algorithm %d",
				bt_mesh_prov_link.oob_method, bt_mesh_prov_link.algorithm);
		prov_fail(PROV_ERR_NVAL_FMT);
		return;
	}
...
static struct k_work dh_gen_work = Z_WORK_INITIALIZER(prov_dh_key_gen_handler);
...
int bt_mesh_prov_enable(bt_mesh_prov_bearer_t bearers)
{
	...
	if ((mesh_log_mask[CONFIG_BT_MESH_PROVISIONEE_LOG_LEVEL >> 1] >> 4) > 2) {
