		...
		uint8_t *p, uint8_t out[32])
{
	...
	err = bt_mesh_sha256_hmac_one_raw_key(t, p, strlen((char *)p), out);
	if (err) {
		return err;
	}
			...
			uint32_t iv_index, enum bt_mesh_nonce_type type)
{
	...
	if ((!IS_ENABLED(CONFIG_BT_MESH_PROXY) && type == BT_MESH_NONCE_PROXY) ||
        (!IS_ENABLED(CONFIG_BT_MESH_SOLICITATION) && type == BT_MESH_NONCE_SOLICITATION)) {
		LOG_DBG("Feature is not enabled with the requested type %d", type);
		return -EINVAL;
	}
		...
		const uint8_t *conf_salt, uint8_t *conf_key)
{
	if (algorithm == BT_MESH_PROV_AUTH_HMAC_SHA256_AES_CCM &&
		...
		return bt_mesh_k5(k_input, 64, conf_salt, (uint8_t *)"prck256", conf_key);
	}
