...
void bt_mesh_model_foreach(model_func func, void *user_data)
{
	int i, j;
				   ...
				   size_t *offset, uint8_t sig_offset)
{
	...
	ext_mod_cnt = count_mod_ext(mod, (uint8_t*)&max_offset, sig_offset);
...
static int publish_transmit(const struct bt_mesh_model *mod)
{
	...
	uint8_t net_buf_data_sdu[BT_MESH_TX_SDU_MAX];
	struct net_buf_simple sdu = {
		.data   = net_buf_data_sdu,
		.len    = 0,
		.size   = BT_MESH_TX_SDU_MAX,
		.__buf  = net_buf_data_sdu,
	};
...
void bt_mesh_msg_cb_set(mesh_msg_cb cb)
{
	msg_cb = cb;
}
#endif
...
void bt_mesh_model_extensions_walk(const struct bt_mesh_model *model, mesh_walk_cb cb,
				   void *user_data)
{
...
static int pre_sig_mod_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return sig_mod_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_sig_mod = {
	.name = "s",
	.h_get = NULL,
	.h_set = pre_sig_mod_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_vnd_mod_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

  return vnd_mod_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_vnd_mod = {
	.name = "v",
	.h_get = NULL,
	.h_set = pre_vnd_mod_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_comp_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return comp_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_comp = {
	.name = "cmp",
	.h_get = NULL,
	.h_set = pre_comp_set,
	.h_commit = NULL,
			    ...
			    const char *key, char *path, size_t path_len)
{
	...
	if (vnd) {
		snprintf(path, path_len, "v/%x/%s", mod_key, key);
	} else {
		snprintf(path, path_len, "s/%x/%s", mod_key, key);
	}
}
...
static void store_pending_mod_sub_va(const struct bt_mesh_model *mod, bool vnd)
{
	...
	if (err) {
		LOG_ERR("Failed to store or delete %s value, err %d", path, err);
	} else {
		LOG_DBG("Stored %s value", path);
	}
#endif /* CONFIG_BT_MESH_LABEL_COUNT > 0 */
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_metadata_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return metadata_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_metadata = {
	.name = "metadata",
	.h_get = NULL,
	.h_set = pre_metadata_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
int bt_mesh_models_metadata_store(void)
{
	...
	uint8_t net_buf_data_buf[CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN];
	struct net_buf_simple buf = {
		.data   = net_buf_data_buf,
		.len    = 0,
		.size   = CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN,
		.__buf  = net_buf_data_buf,
	};
	...
	LOG_DBG("metadata total %d", total_size);
	...
	LOG_DBG("metadata len %d", buf.len);

	err = settings_save_one("metadata", buf.data, buf.len);
	if (err) {
...
int bt_mesh_models_metadata_read(struct net_buf_simple *buf, size_t offset)
{
	...
	uint8_t net_buf_data_stored_buf[CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN];
	struct net_buf_simple stored_buf = {
		.data   = net_buf_data_stored_buf,
		.len    = 0,
		.size   = CONFIG_BT_MESH_MODELS_METADATA_PAGE_LEN,
		.__buf  = net_buf_data_stored_buf,
	};
	...
	err = settings_load_subtree_direct("metadata", read_comp_cb, &stored_buf);
	if (err) {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_access_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_sig_mod);
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_vnd_mod);
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_comp);
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_metadata);
}
#endif
