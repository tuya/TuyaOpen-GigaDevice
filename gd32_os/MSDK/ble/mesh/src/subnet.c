...
static struct bt_mesh_subnet_cb *p_subnet_list = NULL;
#else
extern uint32_t _subnet_cbs[];
extern uint32_t _esubnet_cbs[];
#endif
...
{
#if (CONFIG_MESH_CB_REGISTERED)
	struct bt_mesh_subnet_cb *p_cur_subnet_cb = p_subnet_list;

	while(p_cur_subnet_cb) {
		p_cur_subnet_cb->evt_handler(sub, evt);
		p_cur_subnet_cb = p_cur_subnet_cb->next;
	}
#else
	struct bt_mesh_subnet_cb *p_cur_subnet_cb = (struct bt_mesh_subnet_cb *)_subnet_cbs;

	while(p_cur_subnet_cb != (struct bt_mesh_subnet_cb *)_esubnet_cbs) {
		p_cur_subnet_cb->evt_handler(sub, evt);
		p_cur_subnet_cb++;
	}
#endif
...
static void clear_net_key(uint16_t net_idx)
{
	...
	snprintf(path, sizeof(path), "NetKey/%x", net_idx);
...
#if (CONFIG_BT_MESH_GATT_PROXY || CONFIG_BT_MESH_PROXY_CLIENT)
...
#if (CONFIG_MESH_CB_REGISTERED)
static int pre_subnet_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return net_key_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_subnet = {
	.name = "NetKey",
	.h_get = NULL,
	.h_set = pre_subnet_set,
	.h_commit = NULL,
	.h_export = NULL,
	.next = NULL,
};
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_subnet_cb_register(struct bt_mesh_subnet_cb *p_subnet_cb)
{
	struct bt_mesh_subnet_cb  *p_cur_subnet_cb = NULL;
	if (p_subnet_cb) {
		if (p_subnet_list == NULL) {
			p_subnet_list = p_subnet_cb;
			p_subnet_cb->next = NULL;
		}
		else {
			p_cur_subnet_cb = p_subnet_list;
			while(p_cur_subnet_cb->next != NULL) {
				p_cur_subnet_cb = p_cur_subnet_cb->next;
			}
			p_cur_subnet_cb->next = p_subnet_cb;
			p_subnet_cb->next = NULL;
		}
	}
}

void bt_mesh_subnet_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_subnet);
}

extern void bt_mesh_app_keys_subnet_cb_init(void);
extern void bt_mesh_proxy_cli_subnet_cb_init(void);
extern void bt_mesh_rpr_srv_subnet_cb_init(void);
extern void bt_mesh_brg_cfg_subnet_cb_init(void);

void bt_mesh_subnet_init(void)
{
	bt_mesh_app_keys_subnet_cb_init();
	bt_mesh_beacon_subnet_cb_init();
#if (CONFIG_BT_MESH_FRIEND)
	bt_mesh_friend_subnet_cb_init();
#endif
#if (CONFIG_BT_MESH_LOW_POWER)
	bt_mesh_lpn_subnet_cb_init();
#endif
#if (CONFIG_BT_MESH_PROXY_CLIENT)
	bt_mesh_proxy_cli_subnet_cb_init();
#endif
#if (CONFIG_BT_MESH_GATT_PROXY)
	bt_mesh_proxy_srv_subnet_cb_init();
#endif
#if (CONFIG_BT_MESH_RPR_SRV)
	bt_mesh_rpr_srv_subnet_cb_init();
#endif
#if (CONFIG_BT_MESH_BRG_CFG_SRV)
	bt_mesh_brg_cfg_subnet_cb_init();
#endif
}
#endif
