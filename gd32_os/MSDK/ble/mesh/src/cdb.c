...
static int pre_cdb_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return cdb_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_cdb = {
	.name = "cdb",
	.h_get = NULL,
	.h_set = pre_cdb_set,
	.h_commit = NULL,
...
static void store_cdb_node(const struct bt_mesh_cdb_node *node)
{
	...
	snprintf(path, sizeof(path), "cdb/Node/%x", node->addr);
...
static void clear_cdb_node(uint16_t addr)
{
	...
	snprintf(path, sizeof(path), "cdb/Node/%x", addr);
...
static void store_cdb_subnet(const struct bt_mesh_cdb_subnet *sub)
{
	...
	snprintf(path, sizeof(path), "cdb/Subnet/%x", sub->net_idx);
...
static void clear_cdb_subnet(uint16_t net_idx)
{
	...
	snprintf(path, sizeof(path), "cdb/Subnet/%x", net_idx);
	err = settings_delete(path);
	if (err) {
...
static void store_cdb_app_key(const struct bt_mesh_cdb_app_key *app)
{
	...
	snprintf(path, sizeof(path), "cdb/AppKey/%x", app->app_idx);
...
static void clear_cdb_app_key(uint16_t app_idx)
{
	...
	snprintf(path, sizeof(path), "cdb/AppKey/%x", app_idx);
	err = settings_delete(path);
	if (err) {
...
static void store_cdb_pending_net(void)
{
	...
	err = settings_save_one("cdb/Net", &net, sizeof(net));
	if (err) {
...
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_cdb);
}
#endif
