...
static struct k_work node_reset_pending = Z_WORK_INITIALIZER(node_reset_pending_handler);
...
static struct bt_mesh_app_key_cb cfg_srv_app_key_cb = {
	.evt_handler = app_key_evt,
	.next = NULL,
};
#else
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_cfg_srv_app_key_reg_init(void)
{
	bt_mesh_app_key_cb_register(&cfg_srv_app_key_cb);
}
#endif
