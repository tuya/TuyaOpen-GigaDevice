...
static int pre_cfg_set(const char *name, size_t len_rd, settings_read_cb read_cb, void *cb_arg)
{
	if (!atomic_test_bit(bt_mesh.flags, BT_MESH_INIT)) {
		return 0;
	}

	return cfg_set(name, len_rd, read_cb, cb_arg);
}

static struct settings_handler_static settings_handler_bt_mesh_cfg = {
	.name = "Cfg",
	.h_get = NULL,
	.h_set = pre_cfg_set,
	.h_commit = NULL,
...
static void store_pending_cfg(void)
{
	...
	err = settings_save_one("Cfg", &val, sizeof(val));
	if (err) {
...
#if (CONFIG_MESH_CB_REGISTERED)
void bt_mesh_cfg_settings_init(void)
{
	bt_mesh_settings_cb_register(&settings_handler_bt_mesh_cfg);
}
#endif
