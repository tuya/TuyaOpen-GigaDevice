...
void hnd_get_time(coap_resource_t *resource, coap_session_t  *session,
    ...
    response->code = my_clock_base ? COAP_RESPONSE_CODE_CONTENT : COAP_RESPONSE_CODE_NOT_FOUND;
...
void init_coap_resources(coap_context_t *ctx) {
    coap_resource_t *r;
#if 0
...
void server_coap_init(void) {
    int opt;
    coap_log_t log_level = COAP_LOG_DEBUG;
    ...
    node.length = 7;
    scheme_hint_bits = coap_get_available_scheme_hint_bits(use_psk[0], 0, COAP_PROTO_NONE);
    ...
    if (have_ep == 0) {
        coap_log_warn("Failed to initialize endpoint\r\n");
        goto exit;
    }
#ifndef HAVE_MALLOC
