 ...
 */
#if 0
static const int ciphersuite_preference_default[] =
{
...
#if 1//defined(MBEDTLS_SSL_CIPHERSUITES)
#define MAX_CIPHERSUITES    sizeof( ciphersuite_definitions     ) /         \
                            sizeof( ciphersuite_definitions[0]  )
static int ciphersuite_preference[MAX_CIPHERSUITES];
static unsigned int ciphersuite_pref_count = 0;

void mbedtls_cs_pref_clear(void)
{
    ciphersuite_pref_count = 0;
    memset((void *)ciphersuite_preference, 0, sizeof(ciphersuite_preference));
}
void mbedtls_cs_pref_add(int id)
{
    ciphersuite_preference[ciphersuite_pref_count++] = id;
}
void mbedtls_cs_pref_dump(void)
{
    ...
    for(i=0; i<print_n; i++)
    {
        printf("\r\n\t[%d]: 0x%x - %s", i, ciphersuite_preference[i], mbedtls_ssl_get_ciphersuite_name(ciphersuite_preference[i]));
    }
    printf("\r\n");
}
...
{
    if (ciphersuite_pref_count == 0)
    {
        int i;
        memset(ciphersuite_preference, 0, sizeof(ciphersuite_preference));
        for(i = 0; i < MAX_CIPHERSUITES; i++)
        {
            ciphersuite_preference[i] = ciphersuite_definitions[i].id;
            ciphersuite_pref_count += 1;
        }
    }
